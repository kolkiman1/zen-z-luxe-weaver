...
const MegaMenu = ({ isScrolled, currentPath }: MegaMenuProps) => {
  ...
  return (
    <nav className="hidden lg:flex items-center gap-6">
      <Link
        to="/"
        className={`luxury-underline text-sm font-body tracking-wide micro-ring ${
          currentPath === '/'
            ? 'text-primary'
      ...
      {navCategories.map((category) => (
        ...
        >
          <Link
            to={category.href}
            className={`flex items-center gap-1 text-sm font-body tracking-wide micro-ring ${
          ...
          <AnimatePresence>
            {activeCategory === category.name && (
              ...
              >
                <div className="min-w-[720px] rounded-xl border border-border shadow-2xl overflow-hidden bg-card">
                      ...
                      <ul className="space-y-2">
                        {category.subcategories.map((sub) => (
                          <li key={sub.name}>
                            <Link
                              to={sub.href}
                              className="text-sm text-foreground/80 hover:text-primary block micro-ring"
                              style={{ transitionProperty: 'transform,color', transitionDuration: 'var(--motion-fast)', transitionTimingFunction: 'var(--motion-ease-out)' }}
                            >
                              {sub.name}
                            </Link>
                          </li>
                        ...
                        <li className="pt-2 border-t border-border/30 mt-3">
                          <Link
                            to={category.href}
                            className="text-sm font-medium text-primary hover:underline micro-ring"
                          >
